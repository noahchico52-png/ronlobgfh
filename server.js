// server.js - Live Cloudflare Worker with Print + All + Kick
    ...
    if (url.pathname === '/api/kick' && request.method === 'POST') {
      ...
      try {
        targetWs.send(JSON.stringify({
          type: 'kick',
          data: { message: 'You have been kicked!' }
        }));
      } catch (err) {}
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .btn-row button { padding: 10px 20px; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: bold; flex: 1; min-width: 100px; }
