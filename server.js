// server.js - Simple Working Player Tracker
    ...
    async function fetchPlayers() {
      try {
        ...
        list.innerHTML = data.map(p => \`
          <div class="player">
            <span>👤 \${p.name} \${p.isOwner ? '👑' : ''}</span>
            <span style="color:#818cf8;">🆔 \${p.userId}</span>
            <span style="background:#2d2d44;padding:2px 12px;border-radius:12px;color:#f472b6;">⚡ \${p.executor}</span>
          </div>
        \`).join('');
